...
async function create_calendar_event(businessId, eventDetails) {
  ...
  try {
    ...
    const businessHoursResult = await module.exports.get_business_hours(businessId);
    ...
    const dayKey = dayNames[new Date(eventDetails.date).getDay()];
    ...
    if (eventDetails.appointmentTypeId) {
      ...
      if (appointmentTypes) {
        appointmentDuration = appointmentTypes.duration;
      }
    }
