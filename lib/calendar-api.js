...
function initializeCalendarClient(refreshToken, accessToken) {
  ...
  if (!clientId || clientId === 'YOUR_CLIENT_ID' || 
    ...
    throw new Error('Las credenciales de Google OAuth (***REMOVED***, ***REMOVED***) no están configuradas correctamente');
  }
...
async function check_calendar_availability(businessId, date) {
  ...
  try {
    ...
    if (!credentials) {
      console.log(`⚠️ Sin credenciales de calendario disponibles, utilizando simulación`);
      // Devolver datos simulados para permitir pruebas sin credenciales reales
    ...
    try {
      ...
      console.log('ℹ️ Usando disponibilidad simulada debido al error de API');
      // Devolver datos simulados si hay error en la API de Google
      return generateMockAvailability(date);
    }
  } catch (error) {
...
/**
 * Genera datos simulados de disponibilidad para pruebas
 * @param {string} date - Fecha en formato YYYY-MM-DD
 * @returns {Object} - Datos simulados de disponibilidad
 */
function generateMockAvailability(date) {
  console.log(`🔄 Generando disponibilidad simulada para fecha ${date}`);
  
  // Crear horarios simulados
  const availableSlots = [];
  const targetDate = new Date(date);
  
  // Para fines de prueba, hacemos que las horas pares estén disponibles
  for (let hour = DEFAULT_BUSINESS_HOURS.start; hour < DEFAULT_BUSINESS_HOURS.end; hour += 2) {
    availableSlots.push({
      time: `${hour}:00`,
      display: `${hour}:00${hour < 12 ? ' AM' : ' PM'}`
    });
...
async function create_calendar_event(businessId, eventDetails) {
  ...
  try {
    ...
    if (!credentials) {
      ...
      return generateMockEventCreation(businessId, eventDetails);
    }
    ...
    try {
      ...
      console.error('⚠️ Error en la conexión con Google Calendar:', googleError);
      console.log('ℹ️ Usando simulación debido al error de API');
      // Simular si hay error en la API de Google
      return generateMockEventCreation(businessId, eventDetails);
    }
  } catch (error) {
...
/**
 * Genera un evento simulado para pruebas
 * @param {string} businessId - ID del negocio
 * @param {Object} eventDetails - Detalles del evento
 * @returns {Object} - Datos simulados del evento creado
 */
function generateMockEventCreation(businessId, eventDetails) {
  console.log(`🔄 Generando creación de evento simulada`, eventDetails);
  
  // Generar ID simulado para el evento
  const mockEventId = `mock-event-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
  const startDateTime = new Date(`${eventDetails.date}T${eventDetails.time}`);
  const endDateTime = new Date(startDateTime);
  endDateTime.setMinutes(endDateTime.getMinutes() + DEFAULT_APPOINTMENT_DURATION);
  ...
  try {
    // Crear datos básicos del evento sin el campo simulation
    ...
    // Marcar como evento simulado en los metadatos pero sin usar el campo simulation
    const metadata = { is_mock: true };
    
    // Guardar en la tabla sin el campo simulation
    supabase
      .from('calendar_events')
      .insert(eventData)
      .then(({ error }) => {
        if (error) {
          console.error('⚠️ Error guardando evento simulado en BD:', error.message);
        } else {
          console.log('✅ Evento simulado guardado en BD');
        }
      });
  } catch (dbError) {
    console.error('⚠️ Error guardando evento simulado:', dbError);
  }
  ...
  return {
    ...
    start_time: startDateTime.toISOString(),
    end_time: endDateTime.toISOString(),
    title: eventDetails.title || `Cita con ${eventDetails.name || eventDetails.phone}`,
    date: eventDetails.date,
    time: eventDetails.time,
    simulation: true
  };
...
async function get_month_availability(businessId, startDate, endDate) {
  try {
    ...
    if (!credentials || !credentials.token) {
      console.log(`⚠️ No hay token de autenticación disponible para el negocio ${businessId}`);
