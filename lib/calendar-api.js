...
const relativeDateUtils = require('../utils/relative-date');
...
async function create_calendar_event(businessId, eventDetails) {
  ...
  try {
    ...
    // Crear fecha con zona horaria de México usando Luxon (más confiable para zonas horarias)
    const appointmentDateLuxon = DateTime.fromISO(`${eventDetails.date}T00:00:00`, { zone: 'America/Mexico_City' });
    const dayKey = dayNames[appointmentDateLuxon.weekday % 7];
    
    console.log(`📅 Validando día de semana para cita: ${eventDetails.date} corresponde a ${dayKey} en zona México (usando Luxon)`);
    console.log(`📅 Fecha Luxon detalles: ${appointmentDateLuxon.toISO()} / Día semana: ${appointmentDateLuxon.weekday}`);
    ...
    try {
      ...
      // Crear fecha y hora de inicio en zona horaria de México usando Luxon
      // Luxon maneja mejor las zonas horarias que los objetos Date nativos
      const startDateTime = DateTime.fromISO(`${eventDetails.date}T${eventDetails.time}:00`, { zone: 'America/Mexico_City' });
      console.log(`📅 Fecha y hora original para cita: ${eventDetails.date}T${eventDetails.time}:00`);
      
      if (!startDateTime.isValid) {
        console.error('❌ Fecha/hora inválida:', startDateTime.invalidReason, startDateTime.invalidExplanation);
        return { 
          success: false, 
      ...
      // Convertir a formato ISO para APIs
      const startDateTimeISO = startDateTime.toISO();
      console.log(`📅 Hora de inicio calculada (Luxon ISO): ${startDateTimeISO}`);
      console.log(`📅 Zona horaria usada: ${startDateTime.zoneName}`);
      ...
      console.log(`📅 Hora de fin calculada (Luxon ISO): ${endDateTimeISO}`);

      // Verificar empalmes: contar cuántas citas existen en ese horario
      // Usar directamente los ISO strings generados por Luxon
      ...
      // Crear el evento usando ISO strings de Luxon
      const event = {
        summary: eventTitle,
...
async function find_customer_appointments(businessId, phoneNumber) {
  ...
  try {
    ...
    // Filtrar solo citas futuras usando Luxon para consistencia en manejo de zona horaria
    const now = relativeDateUtils.getNowInMexico();
    const today = now.toISODate(); // YYYY-MM-DD en zona horaria de México
    ...
    const futureEvents = events.filter(event => {
      ...
      if (event.event_date === today) {
        ...
        // Usar hora actual en zona México
        return (eventHour > now.hour || 
                (eventHour === now.hour && eventMinute > now.minute));
      }
      return false;
    });
...
async function get_assistant_calendar_info(businessId, date = null) {
  try {
    ...
    const tomorrowFormatted = relativeDate.getTomorrowInMexico();
    
    const dayAfterTomorrow = new Date(mexicoDate);
    dayAfterTomorrow.setDate(mexicoDate.getDate() + 2);
    const dayAfterTomorrowFormatted = relativeDate.getDayAfterTomorrowInMexico();
    
    const nextWeek = new Date(mexicoDate);
    nextWeek.setDate(mexicoDate.getDate() + 7);
    const nextWeekFormatted = relativeDate.getNextWeekInMexico();
    ...
    return {
      ...
      relative_dates: {
        tomorrow: {
          ...
          formatted: `${DateTime.fromISO(tomorrowFormatted).setZone('America/Mexico_City').day} de ${monthNames[DateTime.fromISO(tomorrowFormatted).setZone('America/Mexico_City').month - 1]} de ${DateTime.fromISO(tomorrowFormatted).setZone('America/Mexico_City').year}`,
          day_name: dayNames[DateTime.fromISO(tomorrowFormatted).setZone('America/Mexico_City').weekday % 7]
        },
        day_after_tomorrow: {
          iso: dayAfterTomorrowFormatted,
          formatted: `${DateTime.fromISO(dayAfterTomorrowFormatted).setZone('America/Mexico_City').day} de ${monthNames[DateTime.fromISO(dayAfterTomorrowFormatted).setZone('America/Mexico_City').month - 1]} de ${DateTime.fromISO(dayAfterTomorrowFormatted).setZone('America/Mexico_City').year}`,
          day_name: dayNames[DateTime.fromISO(dayAfterTomorrowFormatted).setZone('America/Mexico_City').weekday % 7]
        },
        next_week: {
          iso: nextWeekFormatted,
          formatted: `${DateTime.fromISO(nextWeekFormatted).setZone('America/Mexico_City').day} de ${monthNames[DateTime.fromISO(nextWeekFormatted).setZone('America/Mexico_City').month - 1]} de ${DateTime.fromISO(nextWeekFormatted).setZone('America/Mexico_City').year}`,
          day_name: dayNames[DateTime.fromISO(nextWeekFormatted).setZone('America/Mexico_City').weekday % 7]
        }
      }
    };
...
/**
 * Obtiene todas las citas programadas para un negocio
 * Devuelve: nombre del cliente, tipo de cita (nombre), hora, teléfono y fecha
 */
async function get_all_appointments(businessId) {
  try {
    const supabase = getSupabaseClient();
    // Traer todas las citas y hacer join con appointment_types
    const { data, error } = await supabase
      .from('calendar_events')
      .select(`
        id,
    ...
    // Mapear para devolver los campos requeridos
    const appointments = data.map(event => ({
      id: event.id,
      event_id: event.event_id,
      date: event.event_date,
      time: event.event_time,
      customer_name: event.customer_name || 'Sin nombre',
      customer_phone: event.customer_phone,
      appointment_type: event.appointment_types?.name || 'Sin tipo'
    }));
